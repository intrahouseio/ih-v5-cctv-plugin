...
function parse(text) {
  ...
  return { headers: parseHeaders(parse[0]), body: parseBody(parse[1]), bodyLength: parseBodyLength(parse[1])  };
}

function checkrawdata(data) {
  const text = String(data).replace('Content-length', 'Content-Length');
...
class Rtsp extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.pps_mode = options.check_pps;
    this.sps_is_spd = false;
    this.sps_7 = null;
    this.sps_8 = null;
  ...
  check_pps(id, data) {
    ...
    if (this.sps_is_spd === false) {
      if (this.sps_7 === null && type === 7) {
        this.sps_7 =  data.slice(12).toString('base64');
      }
      if (this.sps_8 === null && type === 8) {
        this.sps_8 = data.slice(12).toString('base64');
      }

      if (this.sps_is_spd === false && this.sps_7 !== null && this.sps_8 !== null) {
        this.sps_is_spd = true;
        this.pps = Buffer.from(this.sps_8, 'base64').toString('hex');

        this.rawdata = this.rawdata.map(i => {
          if (i.indexOf('application/sdp') !== -1) {
            if (i.indexOf('a=fmtp:') !== -1) { 
              return i.split('\r\n').map(row => {
                if (row.indexOf('a=fmtp:') !== -1 && i.indexOf('sprop-parameter-sets=') === -1) {
                  return row + '; sprop-parameter-sets=' + this.sps_7 + ',' + this.sps_8;
                }
                return row
              }).join('\r\n');
            } else {
              return i.split('\r\n').map(row => {
                if (row.indexOf('a=rtpmap:96') !== -1) {
                  return row + '\r\n' + 'a=fmtp:96'+ ' sprop-parameter-sets=' + this.sps_7 + ',' + this.sps_8;
                }
                return row
              }).join('\r\n');
            }
    ...
    if (this.pps_mode) {
      ...
      if (this.sps_is_spd === true && this.rawdata_mode === 0) {
        if (type === 8) {
          const pps = data.slice(12).toString('hex');
          if (this.pps === pps) {
            this.rawdata_mode = 1;
  ...
  getAuth() {
    if (this.auth && this.login !== '') {
      if (this.digestrealm === '' && this.nonce === '') {
        ...
        const url = this.method === 'SETUP' ? this.url + this.uri : this.url;
        const a = crypto.createHash('md5').update(`${this.login}:${this.digestrealm}:${this.pass}`).digest("hex");
        const b = crypto.createHash('md5').update(`${this.method}:${url}`).digest("hex");
        const c = crypto.createHash('md5').update(`${a}:${this.nonce}:${b}`).digest("hex");
        const d = `Digest username="${this.login}", realm="${this.digestrealm}", nonce="${this.nonce}", uri="${url}", response="${c}"`;
  ...
  responseOK(msg, data) {
    ...
    if (this.method === 'DESCRIBE') {
      if (msg.bodyLength >= msg.headers['content-length']) {
        this.isDescribeChunked = false;
        this.bufDescribe = [];
      } else {
        this.isDescribeChunked = true;
        this.bufDescribe = this.bufDescribe.concat(data);
      }

      if (this.isDescribeChunked === false) {
        this.rawdata.push(checkrawdata(data))
        let check = false;
  ...
  message(data) {
    ...
    if (data.length >= 4) {
      if (this.isDescribeChunked || (data[0] === 82 && data[1] === 84 && data[2] === 83 && data[3] === 80)) {
        if (this.isDescribeChunked) {
          data = this.bufDescribe.concat(data);
        }
        const msg = parse(String(data));
        if (msg.headers.code === '200') {
          this.responseOK(msg, data);
        } else {
          this.responseError(msg);
        }
        const isEnd = data.indexOf('\r\n\r\n');
        if (isEnd !== -1 && isEnd + 4 !== data.length && this.method !== 'SETUP') {
          this.message(data.slice(isEnd + 4))
        }
